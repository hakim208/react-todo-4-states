...
import { deleteUser } from '@/store/redux/features/todos/deleteTodo'
import { updateStatus } from '@/store/redux/features/todos/updateStatus'
import "swiper/css"
import { Swiper, SwiperSlide } from "swiper/react";
import { Autoplay } from "swiper/modules"
import { updateUser } from '@/store/redux/features/todos/updateUser'
import { deleteImageTodo } from '@/store/redux/features/todos/deleteImageTodo'
import { addImageTodo } from '@/store/redux/features/todos/addImageTodo'
import { addUserTodo } from '@/store/redux/features/todos/addUserTodo'
...
const Async = () => {
  ...
  async function closeAddModal() {
    await dispach(addUserTodo(form))
    setOpenAdd(false)
    setaddName("")
    setaddDescription("")
    setImage([])
  ...
  async function closeModal() {
    let obj = {
      ...
      isCompleted: editSattus,
      id: idx
    }
    await dispach(updateUser(obj))
    setOpen(false)
  }
              ...
              <TableRow key={row.id}>
                ...
                <TableCell align="right">
                  ...
                  >
                    ...
                    <MenuItem
                      onClick={() => { dispach(deleteUser(row.id)); handleClose(row.id); }}
                      sx={{
                        display: "flex",
                        alignItems: "center",
                        gap: 1,
                    ...
                    <MenuItem
                      onClick={() => { dispach(updateStatus(row.id)); handleClose(row.id); }}
                      sx={{
                        display: "flex",
                        alignItems: "center",
                        gap: 1,
                    ...
                    <MenuItem
                      onClick={() => { { row.images && row.images.map((e) => dispach(deleteImageTodo(e.id))) }; handleClose(row.id); }}
                      sx={{
                        display: "flex",
                        alignItems: "center",
                        gap: 1,
